...
def lgb_train(data, target, ltr, train_cols, split_list, param, n_e = 10000, cat_col = None, verb_num = None, imp=False):
    ...
    for i , (train_index, test_index) in enumerate(split_list):
        ...
        sc = StandardScaler()
        param['seed'] = i
        tr = lgb.Dataset(sc.fit_transform(np.array(data[train_cols])[train_index]), np.array(data[target])[train_index])
        te = lgb.Dataset(sc.fit_transform(np.array(data[train_cols])[test_index]), np.array(data[target])[test_index], reference=tr)
        tt = lgb.Dataset(sc.fit_transform(np.array(data[train_cols]))[ltr:, :])
        ...
        pred[str(i)] =bst.predict(sc.fit_transform(np.array(data[train_cols])[ltr:]))
        pred_val[test_index] = bst.predict(sc.fit_transform(np.array(data[train_cols])[test_index]))
