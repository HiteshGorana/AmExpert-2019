...
test_ = [np.log1p(np.absolute(test[f].values)) for f in train_cols]
...
for i , (tdx, vdx) in enumerate(kf.split(train, train.redemption_status)):
    ...
    print(f"FOLD : {i}")
    X_train = [np.log1p(np.absolute(train[f].iloc[tdx].values)) for f in train_cols]
    X_test = [np.log1p(np.absolute(train[f].iloc[vdx].values)) for f in train_cols]
